$(document).ready(function () {
    ...
    //     "<select class='select' id='table-action'>" +
    ...
    function checkMoreAction() {
        if (table.rows({ selected: true }).indexes().length > 0) {
            $(".dataTables_filter select#table-action")
                .find("option[value='1']")
                .prop("disabled", false);
            $(".dataTables_filter select#table-action")
                .find("option[value='2']")
    ...
    function confirm(table, option, url, type) {
        var title =
            option == 2
                ? "Are you sure you want to deactivate selected item(s) ?"
                : "Are you sure you want to activate selected item(s) ?";
        var text =
            option == 2
                ? "You will be able to undo your action later!"
                : "You will be able to undo your action later!";
        var conf = option == 2 ? "Yes, deactive!" : "Yes, activate!";
        var canc =
            option == 2
                ? "No, cancel deactive please!"
                : "No, cancel activating please!";
